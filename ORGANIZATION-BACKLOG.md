...
- Первичный набор Workspace issues удалён по решению владельца до начала разработки.
- Project намеренно оставлен пустым: искусственный backlog не заменяет задачи, сформированные по
  Developer Pipeline.
- Первая реальная issue будет добавлена repository automation после её создания или повторного
  открытия.
- Готово, когда Project отражает только реальную текущую работу.
