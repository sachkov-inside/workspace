...
рекомендацией следующей, но не начинает её сам. Просьба, в которой названо несколько стадий или их
диапазон («доведи до Implementation»), запускает каждую по порядку. Согласование, которое требует
gate или skill внутри стадии, по-прежнему ждёт владельца, если в просьбе его не было. Правило
действует в любом рантайме, в том числе если рантайм не соблюдает признак user-only у skill.
...
разрешает credentialed/external writes, выпуски и каждый merge. Готовую `ready-for-agent` задачу
агент выполняет автономно внутри этих границ и возвращает outcome, caveats, verification и ссылки на
durable artifacts.

Данное согласие действует в названных владельцем пределах, пока он его не отзовёт: агент не
спрашивает повторно, а уточнение владельца дополняет текущую задачу. Согласие на merge можно дать
заранее на названные задачи — тогда агент мержит их PR, когда они готовы по `WORKFLOW.md`. Всё,
что за пределами согласия, по-прежнему требует отдельного решения; ожидание ответа согласием не
является.
